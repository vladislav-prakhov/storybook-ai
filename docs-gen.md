...
**Then sanitize for the template literal** (the description is inserted inside a ``...``
string later): replace every backtick ``` with a single quote `'`, and write any literal
`${` as `\${`. Do not summarize, shorten, reorder, or rephrase anything else.

The result, `MUI_OVERVIEW`, goes into the story's `description.component` (Step 5).
...
## STEP 4 — Build argTypes

For every prop, produce one `argType` entry. **Split them into two labelled groups** — this
separation is required and must be visible in the file:

* **Caldera props:** every prop that appears in `extracted/types.json`.
